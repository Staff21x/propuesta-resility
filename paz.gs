...
// VERSION: v4.1 - Correcciones rendimiento, bug columna G y robustez
// ============================================================

function configurarKeys() {
  const props = PropertiesService.getScriptProperties();
  props.setProperty("OPENAI_API_KEY",   "PEGA_ACA_TU_KEY_OPENAI");
  props.setProperty("WASSENGER_TOKEN",  "PEGA_ACA_TU_TOKEN_WASSENGER");
  props.setProperty("EMAIL_STAFF21X",   "PEGA_ACA_TU_EMAIL_STAFF21X");
  props.setProperty("LANDING_OTEC",     "https://www.staff21x.com/otec");
  Logger.log("Keys configuradas. Borra los valores y guarda.");
}

// FIX: getConfig() se llama una vez y se pasa como argumento.
// FIX: EMAIL_STAFF21X y LANDING_OTEC ahora vienen de ScriptProperties.
function getConfig() {
  const props = PropertiesService.getScriptProperties();
  return {
    ...
    SPREADSHEET_ID:  "1-9tjYt1kyGQI-kEONhk-9VLudD8ylg_9e7ptVfTYklg",
    CALENDAR_ID:     "primary",
    MAX_HISTORIAL:   3000
  };
}
...
// ============================================================
function doPost(e) {
  // FIX: config se obtiene una sola vez y se pasa a todas las funciones.
  ...
  try {
    ...
    const numeroCliente  = data.data.fromNumber;
    Logger.log("WSP de: " + numeroCliente + " | Texto: " + textoCliente);

    // FIX: abrimos el spreadsheet una sola vez para todo el flujo de doPost.
...
// FIX: recibe ss y config ya instanciados en lugar de abrirlos de nuevo.
// FIX: valida estructura del JSON de la IA antes de acceder a propiedades.
// FIX: historial consistente con MAX_HISTORIAL en lectura y escritura.
// ============================================================
...
RESPONDE SOLO EN JSON, SIN TEXTO ADICIONAL:
{
  ...
  "respuesta_final": "..."
    ...
    // FIX: validacion defensiva de la estructura del JSON antes de usarla.
    let proceso;
    try {
...
// FIX: recibe config como parametro.
// ============================================================
...
function obtenerProximoSlotDisponible(config) {
  try {
    ...
    // Fallback: proximo lunes a las 10
...
// ============================================================
// LISTADO DE ENVIOS
// FIX: recibe ss como parametro para evitar abrir el spreadsheet de nuevo.
// ============================================================
  ...
  try {
    ...
    for (let i = 1; i < datos.length; i++) {
      ...
      if (telFila && numLimpio.endsWith(telFila.slice(-8))) {
        return datos[i][6] || null; // columna G = EMAIL
      }
    }
  } catch (e) {
...
// FIX: usa Array.some() en lugar de for loop manual.
...
// FIX: corregido bug critico: la columna G (email) ya no se sobreescribe con timestamp.
//      El timestamp de envio va a columna H (nueva columna dedicada).
// ============================================================
function enviarCampanaDiaria() {
  ...
  for (let i = 1; i < datos.length; i++) {
    ...
    const estado = datos[i][5]; // columna F = Estado
    if (estado === "ENVIADO" || estado === "ERROR") continue;

    const nombre   = datos[i][1];
    const telefono = datos[i][2];
    ...
    if (ok) {
      hoja.getRange(i + 1, 6).setValue("ENVIADO");                             // columna F = Estado
      hoja.getRange(i + 1, 8).setValue(new Date().toLocaleString("es-CL"));   // columna H = Fecha envio (era G, bug corregido)
      enviados++;
...
// ============================================================
// MEMORIA
// FIX: obtenerOCrearHojaMemoria recibe ss como parametro.
// FIX: guardarEmailEnMemoria recibe ss y config como parametros.
// FIX: catch con logging en lugar de bloque vacio.
// ============================================================
...
// ============================================================
// WASSENGER Y OPENAI
// FIX: ambas funciones reciben config como parametro.
// ============================================================
function enviarWassenger(config, num, msg) {
  try {
    const res  = UrlFetchApp.fetch("https://api.wassenger.com/v1/messages", {
      method:          "post",
      headers:         {"Token": config.WASSENGER_TOKEN, "Content-Type": "application/json"},
      payload:         JSON.stringify({"phone": num, "message": msg}),
      muteHttpExceptions: true
    });
    const code = res.getResponseCode();
    Logger.log("Wassenger [" + num + "]: " + code);
    return code === 201;
  } catch (e) {
    Logger.log("Error Wassenger: " + e);
    return false;
  }
}
...
function llamarIA(config, sys, user) {
  for (let intento = 0; intento <= 2; intento++) {
    try {
      ...
      Utilities.sleep(2000 * Math.pow(2, intento)); // backoff exponencial
    } catch (e) {
...
function testEnviarUno() {
  const config = getConfig();
  const ok = enviarWassenger(config, "+56987345554", "TEST Staff21x - Paz conectada correctamente.");
